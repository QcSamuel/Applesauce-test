        ...
        if (mNextNativeState == NativeState.RESUMED) {
            if (mSurface.mIsSurfaceReady && mHasFocus && mIsResumedCalled) {
                if (mSDLThread == null) {
                    ...
                    // XaView: give the emulation thread a big stack (16 MiB
                    // intended). The default Java thread stack is far too small
                    // for heavy iOS apps (Asphalt 8 aborts natively during init
                    // before its ES2 context is created).
                    // NOTE: kept byte-identical to the XaView fork, including the
                    // unusual `1 << 20 * 16` expression, because that exact
                    // build is confirmed working on Android.
                    mSDLThread = new Thread(Thread.currentThread().getThreadGroup(), new SDLMain(), "SDLThread", 16 * 1024 * 1024);
